	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			m := mockedSecretsManager{
				...
				describeSecretsPages: tt.pages,
				describeSecretsErr:   tt.err,
			}
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("EC2Svc.Secrets() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("EC2Svc.Secrets() = %v, want %v", got, tt.want)
			}
