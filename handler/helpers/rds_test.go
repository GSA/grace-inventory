...
func TestRDSSvc_DBInstances(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			m := mockedRDS{
				describeDBInstancesPages: tt.dbInsPages,
				describeDBInstancesErr:   tt.descrErr,
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("RDSSvc.DBInstances() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("RDSSvc.DBInstances() = %v, want %v", got, tt.want)
			}
			if !reflect.DeepEqual(m.CallsList(), tt.wantCalls) {
				t.Errorf("Call list mismatch: %v, expected %v", m.CallsList(), tt.wantCalls)
			}
		})
	}
}
...
func TestRDSSvc_DBSnapshots(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			m := mockedRDS{
				describeDBSnapshotsPages: tt.dbSnapPages,
				describeDBSnapshotsErr:   tt.descrErr,
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("RDSSvc.DBSnapshots() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("RDSSvc.DBSnapshots() = %v, want %v", got, tt.want)
			}
			if !reflect.DeepEqual(m.CallsList(), tt.wantCalls) {
				t.Errorf("Call list mismatch: %v, expected %v", m.CallsList(), tt.wantCalls)
			}
